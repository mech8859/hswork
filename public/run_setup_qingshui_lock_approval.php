<?php
/**
 * 一次性腳本：為「清水電子鎖」分公司建立完工簽核三層規則
 *   Level 1 工程主管 - 蕭凱澤
 *   Level 2 行政人員（勾選有無收款）- 楊雨倫
 *   Level 3 會計人員（確認入帳）- 林晏鈴
 *
 * 用法：/run_setup_qingshui_lock_approval.php           （預覽）
 *       /run_setup_qingshui_lock_approval.php?execute=1 （實際執行）
 */
...
foreach (array('蕭凱澤' => $u1, '楊雨倫' => $u2, '林晏鈴' => $u3) as $n => $u) {
...
$rules = array(
    array(
        'level' => 1,
        'name'  => '完工簽核 - 工程主管（清水電子鎖）',
        'role'  => 'eng_manager',
        'user'  => $u1,
    ),
    array(
        'level' => 2,
        'name'  => '完工簽核 - Level 2 行政人員（清水電子鎖，勾選有無收款）',
        'role'  => 'admin_staff',
        'user'  => $u2,
    ),
    array(
        'level' => 3,
        'name'  => '完工簽核 - Level 3 會計人員（清水電子鎖，確認入帳）',
        'role'  => 'accountant',
        'user'  => $u3,
    ),
);
