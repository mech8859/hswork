...
try {
    ...
    $stmt = $db->query("SELECT id, username, employee_id, real_name FROM users WHERE employee_id IS NOT NULL AND employee_id != '' ORDER BY employee_id");
    $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $updated = 0;
    $skipped = 0;
    foreach ($rows as $r) {
        $newUser = 'hs' . $r['employee_id'];
        ...
        if ($chk->fetch()) { echo "SKIP: " . $r['real_name'] . " (" . $r['employee_id'] . ") -> " . $newUser . " exists\n"; $skipped++; continue; }
