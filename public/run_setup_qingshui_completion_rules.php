...
$bStmt->execute(array('QINGSHUI', '%清水%'));
$branch = $bStmt->fetch(PDO::FETCH_ASSOC);
if (!$branch) { die("找不到清水分公司\n"); }
$branchId = (int)$branch['id'];
echo "清水分公司：id={$branchId}, name={$branch['name']}, code={$branch['code']}\n\n";

// 2. 找三位簽核人
$names = array(
    1 => array('role' => 'eng_manager', 'name' => '顏伸祐',   'label' => '工程主管'),
    2 => array('role' => 'admin_staff', 'name' => '楊雨倫',   'label' => '行政人員'),
...
// 3. 建立三條規則
$ruleDefs = array(
    1 => '完工簽核 - 清水分公司 Level 1 工程主管',
    2 => '完工簽核 - 清水分公司 Level 2 行政人員（勾選有無收款）',
    3 => '完工簽核 - 清水分公司 Level 3 會計人員（確認入帳）',
);

foreach ($ruleDefs as $level => $ruleName) {
    // 檢查是否已存在同分公司同 level 的規則
    $chk = $db->prepare("
