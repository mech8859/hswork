<?php
/**
 * 東區電子鎖零用金 — 從 Ragic API 全量重新同步
 * 流程：刪除東區電子鎖既有零用金 → 從 Ragic 全量抓取 → 全部 INSERT
 *
 * 用法:
 *   ?dry=1     ← 預覽模式（預設，不寫入）
...
$url = 'https://ap15.ragic.com/hstcc/east-district-electronic-lock/16?api';
...
// 2. 取得東區電子鎖 ID
$branchStmt = $db->prepare("SELECT id, name FROM branches WHERE name LIKE '%東區%'");
$branchStmt->execute();
...
echo "東區電子鎖 ID: $tanziBranchId ({$branchRow['name']})\n";
...
if (!$execute) {
    ...
    echo '</pre>';
    echo '<a href="?execute=1" onclick="return confirm(\'確定清空東區全部 ' . $existingCount . ' 筆零用金，並從 Ragic 重新匯入 ' . count($ragicData) . ' 筆？\')" style="display:inline-block;padding:10px 20px;background:#e53935;color:#fff;border-radius:6px;text-decoration:none;font-weight:600">執行清空+重新同步</a>';
    exit;
}
