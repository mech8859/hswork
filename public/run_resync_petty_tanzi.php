<?php
/**
 * 潭子分公司零用金 — 從 Ragic API 全量重新同步
 * 流程：刪除潭子分公司既有零用金 → 從 Ragic 全量抓取 → 全部 INSERT
 *
 * 用法:
 *   ?dry=1     ← 預覽模式（預設，不寫入）
...
// 2. 取得潭子分公司 ID
$branchStmt = $db->prepare("SELECT id, name FROM branches WHERE name LIKE '%潭子%'");
$branchStmt->execute();
...
echo "潭子分公司 ID: $tanziBranchId ({$branchRow['name']})\n";
...
if (!$execute) {
    ...
    echo '</pre>';
    echo '<a href="?execute=1" onclick="return confirm(\'確定清空潭子全部 ' . $existingCount . ' 筆零用金，並從 Ragic 重新匯入 ' . count($ragicData) . ' 筆？\')" style="display:inline-block;padding:10px 20px;background:#e53935;color:#fff;border-radius:6px;text-decoration:none;font-weight:600">執行清空+重新同步</a>';
    exit;
}
