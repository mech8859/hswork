...
$jsonPath = '/raid/vhost/hswork.com.tw/ragic_bank_fubon.json';
...
// Only get existing records with changhua bank upload numbers (TC-22- prefix or 富邦)
$sysRecords = $db->query("SELECT * FROM bank_transactions WHERE bank_account LIKE '%富邦%' OR upload_number LIKE 'TC-22-%' ORDER BY id")->fetchAll(PDO::FETCH_ASSOC);
$sysIndex = array();
foreach ($sysRecords as $s) { if (!empty($s['upload_number'])) $sysIndex[$s['upload_number']] = $s; }
echo "System (富邦): " . count($sysRecords) . " records\n\n";
