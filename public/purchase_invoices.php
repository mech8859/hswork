...
switch ($action) {
    ...
    case 'create':
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            $data = array(
                ...
                'buyer_tax_id'       => !empty($_POST['buyer_tax_id']) ? $_POST['buyer_tax_id'] : null,
    ...
    case 'toggle_star':
        ...
        try {
            $db = Database::getInstance();
            $cur = $db->prepare("SELECT is_starred FROM purchase_invoices WHERE id = ?");
            $cur->execute(array($sid));
            $c = $cur->fetchColumn();
            if ($c === false) { echo json_encode(array('error' => '記錄不存在')); exit; }
            $new = ((int)$c === 1) ? 0 : 1;
            $db->prepare("UPDATE purchase_invoices SET is_starred = ? WHERE id = ?")->execute(array($new, $sid));
