...
$jsonPath = '/raid/vhost/hswork.com.tw/ragic_cash_details.json';
...
$sysRecords = $db->query("SELECT * FROM cash_details ORDER BY id")->fetchAll(PDO::FETCH_ASSOC);
$sysIndex = array();
...
foreach ($ragicIndex as $no => $ragic) {
    ...
    $fields = array(
        'entry_number' => $no,
        'register_date' => cleanDate($ragic['entry_date']),
        'transaction_date' => cleanDate($ragic['transaction_date']),
        'branch_id' => $branchId,
        'sales_name' => $ragic['sales_name'] ?: null,
    ...
    if (!isset($sysIndex[$no])) {
        try {
            ...
            $db->prepare("INSERT INTO cash_details (" . implode(',', $cols) . ") VALUES (" . implode(',', $ph) . ")")->execute(array_values($fields));
            $added++;
        ...
        if ($needUpdate) {
            try {
                $updateVals[] = $sys['id'];
                $db->prepare("UPDATE cash_details SET " . implode(',', $updateCols) . " WHERE id = ?")->execute($updateVals);
                $updated++;
...
// 刪除 Ragic 沒有的
foreach ($sysIndex as $no => $sys) {
    if (!isset($ragicIndex[$no])) {
        try {
            $db->prepare("DELETE FROM cash_details WHERE id = ?")->execute(array($sys['id']));
            $deleted++;
        } catch (Exception $e) { $errors[] = "DEL $no: " . $e->getMessage(); }
    }
}
