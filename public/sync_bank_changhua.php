...
$jsonPath = '/raid/vhost/hswork.com.tw/ragic_bank_changhua.json';
...
// Only get existing records with changhua bank upload numbers (TC-21- prefix or 彰化)
$sysRecords = $db->query("SELECT * FROM bank_transactions WHERE bank_account LIKE '%彰化%' OR upload_number LIKE 'TC-21-%' ORDER BY id")->fetchAll(PDO::FETCH_ASSOC);
$sysIndex = array();
foreach ($sysRecords as $s) { if (!empty($s['upload_number'])) $sysIndex[$s['upload_number']] = $s; }
echo "System (彰化): " . count($sysRecords) . " records\n\n";
