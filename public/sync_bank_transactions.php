...
$jsonPath = '/raid/vhost/hswork.com.tw/ragic_bank_transactions.json';
...
// Index by upload_number
$ragicIndex = array();
foreach ($ragicData as $r) {
    if (!empty($r['upload_number'])) $ragicIndex[$r['upload_number']] = $r;
}

$sysRecords = $db->query("SELECT * FROM bank_transactions ORDER BY id")->fetchAll(PDO::FETCH_ASSOC);
$sysIndex = array();
foreach ($sysRecords as $s) {
    if (!empty($s['upload_number'])) $sysIndex[$s['upload_number']] = $s;
}
