...
### 4-3. 全勤（S 欄）/ 全勤病假$（U 欄）/ 全勤病假天數（T 欄）

**資料來源**：規則檔類別 B + 出勤紀錄 L 欄請假時數（病假部分）

**計算公式**：
```
全勤病假$ (U欄) = ROUND( 2000/30 × 全勤病假天數(T欄), 0 )
全勤      (S欄) = 2000 − 全勤病假$
```

**邏輯說明**：
- 滿勤獎金固定 $2,000
- 有病假 → 按比例扣（每 30 天工作日扣 $2,000）
- 例：T=1.5 天 → U = ROUND(2000/30 × 1.5, 0) = $100，S = 2000 − 100 = $1,900

**Excel 公式範例**（套於 r 列）：
```
U: =IF(T{r}=0, 0, ROUND(2000/30*T{r}, 0))
S: =2000 - U{r}
```

> 規則檔已套用為 U 欄公式 `=IF(T4=0,0,ROUND(2000/30,0))`，但**未對 T>1 的情況乘上 T**，目前人工修正。
