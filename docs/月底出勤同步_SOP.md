...
### 4-4. 請假扣款（AA 病假$、AC 事假$、AL 請假扣款）

**資料來源**：規則檔類別 C + 出勤紀錄 L 欄

**計算公式**：
```
病假$ (AA) = ROUND( (底薪I + 職務加給K + 技術加給Q) / 30 × 病假/生理假天數(Z), 0 )
事假$ (AC) = ROUND( (底薪I + 職務加給K + 技術加給Q) / 30 × 事假天數(AB), 0 )
請假扣款 (AL) = AA + AC
```

**特殊規則**：
- **特休不扣全勤、計入出勤日**（AD 欄記特休天數，但不扣 S 欄全勤獎金，H 欄出勤日仍 = 30）

**Excel 公式範例**：
```
AA: =ROUND( ((I{r}+K{r}+Q{r})/30 * Z{r}/2 ), 0 )   ← 注意：目前範本是 Z/2，待確認是否含括號錯誤
AC: =ROUND( (I{r}+K{r}+Q{r})/30 * AB{r}, 0 )
AL: =AA{r} + AC{r}
```

> ⚠️ 目前 AA 公式有 `Z4/2`（除 2），可能是「半薪生理假」的歷史邏輯，請確認是否符合規則。若不符合，改為 `Z` 不除 2。
