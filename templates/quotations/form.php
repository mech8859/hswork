...
function calcGrandTotal() {
    ...
    if (canManage) {
        ...
        // 防禦：若 laborCost 為 0 但 laborHours 有值（DOM 漂移），用 hours × 時薪 重算
        // 注意：laborHours 已是「總人時」(autoCalcHours 中 = 人數×天數×8 或 人數×每人時數)，不可再乘人數
        var _lh = parseFloat(document.getElementById('laborHours').value) || 0;
        if (_lh > 0) {
            var _expected = Math.round(_lh * laborHourlyCost);
