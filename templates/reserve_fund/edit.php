...
<div class="d-flex justify-between align-center flex-wrap gap-1 mb-2">
    ...
    <a href="/reserve_fund.php" class="btn btn-outline btn-sm">&larr; 返回列表</a>
</div>

<div class="card">
    <form method="POST" action="/reserve_fund.php?action=update&id=<?= e($record['id']) ?>">
        <?= csrf_field() ?>
        <div class="edit-form-grid">
            ...
            <div class="form-group">
                <label>登記日期</label>
                <input type="text" class="form-control" value="<?= e(!empty($record['entry_date']) ? $record['entry_date'] : (!empty($record['created_at']) ? substr($record['created_at'], 0, 10) : '-')) ?>" disabled>
            </div>
            <div class="form-group">
                <label>收支別</label>
            ...
            <div class="form-group">
                <label>支出日期</label>
                <input type="date" max="2099-12-31" name="expense_date" class="form-control" value="<?= e(!empty($record['expense_date']) ? $record['expense_date'] : '') ?>" <?= $disabled ?>>
            </div>
            <div class="form-group">
                <label>金額</label>
        ...
        <div class="d-flex gap-1 mt-2">
            ...
            <a href="/reserve_fund.php?action=delete&id=<?= e($record['id']) ?>&csrf_token=<?= e(Session::getCsrfToken()) ?>"
               class="btn btn-danger btn-sm"
               onclick="return confirm('確定要刪除此筆備用金記錄嗎？此操作無法復原。')">刪除</a>
            <?php endif; ?>
            <a href="/reserve_fund.php" class="btn btn-outline btn-sm">返回</a>
        </div>
    </form>
</div>
