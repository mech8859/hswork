    ...
    <form method="GET" action="/goods_receipts.php" class="filter-form">
        <div class="filter-row">
            ...
            <div class="form-group" style="position:relative">
                <label>廠商</label>
                <input type="text" id="grVendorName" autocomplete="off" name="vendor_name" class="form-control" value="<?= e(!empty($filters['vendor_name']) ? $filters['vendor_name'] : '') ?>" placeholder="廠商名稱">
                <div id="grVendorSuggestions" class="gr-vendor-suggestions"></div>
            </div>
            <div class="form-group">
                <label>關鍵字</label>
...
.gr-vendor-suggestions { display:none; position:absolute; top:100%; left:0; right:0; max-height:260px; overflow-y:auto; background:#fff; border:1px solid var(--gray-200); border-radius:var(--radius); box-shadow:var(--shadow); z-index:50; }
.gr-vendor-suggestions.show { display:block; }
.gr-vendor-suggestions .ac-item { padding:8px 12px; cursor:pointer; border-bottom:1px solid var(--gray-100); }
.gr-vendor-suggestions .ac-item:hover { background:var(--gray-50); }
.gr-vendor-suggestions .ac-main { font-weight:600; font-size:.9rem; }
.gr-vendor-suggestions .ac-sub { font-size:.78rem; color:var(--gray-500); }
</style>
<script>
(function() {
    var input = document.getElementById('grVendorName');
    var list = document.getElementById('grVendorSuggestions');
